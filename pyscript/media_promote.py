...
def _format_sonarr_line(tv: list, tv_queue: int, upcoming_days: int) -> str:
    """Format Sonarr L1 line (max 255 chars).

    Example: "Sonarr: Last Week Tonight S13E04, Late Night S13E74 today."
    """
...
def _format_radarr_line(movies: list, movie_queue: int, upcoming_days: int) -> str:
    """Format Radarr L1 line (max 255 chars).

    Example: "Radarr: The Matrix (Mar 15) today."
    """
...
def _format_upcoming_line(
    ...
    if tv:
        tv_names = [item.get("label", item.get("title", "?")) for item in tv[:4]]
        suffix = f" +{len(tv) - 4} more" if len(tv) > 4 else ""
        parts.append(f"Sonarr: {', '.join(tv_names)}{suffix}")
    if movies:
        movie_names = []
        for m in movies[:3]:
            name = m.get("title", "?")
        ...
        suffix = f" +{len(movies) - 3} more" if len(movies) > 3 else ""
        parts.append(f"Radarr: {', '.join(movie_names)}{suffix}")
...
def _format_recent_line(tv_recent: list, movie_recent: list) -> str:
    """Format recent downloads for briefing helper (max 255 chars).
