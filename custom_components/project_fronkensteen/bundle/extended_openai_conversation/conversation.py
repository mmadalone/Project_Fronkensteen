...
# ── Gemini tool-call narration sanitizer ─────────────────────────────────
# Gemini 2.5 Flash returns text alongside function calls. When that text
# is raw call syntax (e.g. 'end_conversation()' or
# 'agent_interaction_log(agent_name="kramer", topic="goodbye")'),
# strip it so TTS doesn't speak function signatures aloud.
...
class ExtendedOpenAIAgentEntity(
    ...
    def _sanitize_for_speech(self, text: str | None) -> str | None:
        ...
        Layer 1: Entire response is a bare function call → return None.
        Layer 2: Response contains known function names → strip them.
        Layer 3: Orphaned keyword arguments (Maverick pattern) → strip
        ...
        if not text:
            return text
        cleaned = text.strip()
        # Layer 1: Entire response is a bare function call
        if _TOOL_CALL_SYNTAX.match(cleaned):
            return None
        # Layer 2: Build pattern from configured function names
        try:
            funcs = self.get_functions() or []
            names = [s["spec"]["name"] for s in funcs if "spec" in s]
        except Exception:
            names = []
        if names:
            pattern = re.compile(
        ...
        # Layer 3: Orphaned tool arguments without function name
        # Maverick leaks: action="promote", library_id="foo")
        if _ORPHAN_TOOL_ARGS.search(cleaned):
            cleaned = _ORPHAN_TOOL_ARGS.sub('', cleaned)
            cleaned = re.sub(r'\s{2,}', ' ', cleaned).strip()
            cleaned = re.sub(r'[,;.\s]+$', '', cleaned).strip()
            cleaned = re.sub(r'^[,;.\s]+', '', cleaned).strip()
        # Layer 4: Inline tool-param sequences (mid-sentence leaks)
        # Catches residual (key="value", ...) after earlier layers strip function names
        if _INLINE_TOOL_PARAMS.search(cleaned):
            cleaned = _INLINE_TOOL_PARAMS.sub(' ', cleaned)
            cleaned = re.sub(r'\s{2,}', ' ', cleaned).strip()
            cleaned = re.sub(r'[,;.\s]+$', '', cleaned).strip()
            cleaned = re.sub(r'^[,;.\s]+', '', cleaned).strip()
    ...
    async def query(
        ...
        request_timeout = self.options.get(CONF_REQUEST_TIMEOUT, DEFAULT_REQUEST_TIMEOUT)
