...
┌─────────────────────────────────────┐
│ Trigger                             │
│  Presence sensor: off → on          │
│  (held for presence_delay)          │
└──────────────┬──────────────────────┘
               │
    ┌──────────▼──────────────────┐
    │ Conditions                  │
...
- Configurable presence delay prevents false triggers
...
- Optional entry script for custom actions (e.g., thermostat to home mode)
...
1. Copy `zone_presence.yaml` to `config/blueprints/automation/madalone/`
...
| `presence_delay` | `0h 5m 0s` | Duration sensor must stay "on" before triggering |
| `master_switch` | `input_boolean.ai_auto_off_enabled` | Global on/off toggle |
| `lights_enabled` | `true` | Turn on lights when zone is occupied |
...
- **Switch handling:** When `use_area_lights` is enabled, `light.turn_on` targets the area. Any `switch.*` entities in `specific_lights` are turned on separately, since `light.turn_on` does not affect switches.
- **Trigger cancellation:** HA natively cancels the `for:` trigger if the sensor returns to "off" before the delay expires
