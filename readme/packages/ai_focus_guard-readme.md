...
| `input_boolean.focus_mode` | input_boolean | Voice-activated DND (suppresses non-calendar nudges) |
| `input_boolean.ai_focus_guard_enabled` | input_boolean | Kill switch (default ON) |
| `input_datetime.last_meal_time` | input_datetime | User marks meals via voice/button |
| `input_datetime.focus_guard_snooze_until` | input_datetime | Snooze expiry timestamp |
| `input_number.focus_guard_threshold_hours` | input_number | Hours before first nudge (default 2) |
| `input_number.focus_mode_max_hours` | input_number | Focus mode auto-expiry (default 2h) |
...
- Focus mode auto-expires using a `for:` duration trigger that reads from `focus_mode_max_hours`, supporting fractional hours.
