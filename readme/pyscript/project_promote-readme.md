...
- `_build_hot_context_line(projects, limit)` -- Build hot context line from sorted projects. Full text stored in `full_text` attribute (state truncated to 255 chars by HA). `@pyscript_compile`.
- `_build_summary_line(projects)` -- Build active/blocked counts + high-priority names. Full text in `full_text` attribute. `@pyscript_compile`.
...
- `sensor.ai_project_hot_context_line` -- Output: top priorities for hot context. `full_text` attribute has untruncated content.
- `sensor.ai_active_projects_summary` -- Output: counts + high-priority names. `full_text` attribute has untruncated content.
...
- **Hot context**: `packages/ai_context_hot.yaml` reads `ai_project_hot_context_line` `full_text` attribute for agent injection
- **Briefing**: `proactive_briefing.py` reads `full_text` attribute via `_section_projects()` for briefing assembly
