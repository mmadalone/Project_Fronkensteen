...
│  77 automation + 35 script blueprints                                   │
...
│  37 Python modules · 155+ services                                      │
│                                                                         │
...
│  helpers_input_*.yaml — user configuration (~225 helpers)               │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| `state_bridge.py` | `set_sensor_value`, `save_budget_state` | Generic bridge service for blueprints that can't call `state.set()`. Seeds 66 sensors on startup. |
...
Helpers are the user's configuration API. ~225 helpers across 3 tiers (Essential, Per-User, Dev Tuning). Pyscript modules and blueprints read them; users and the dashboard write them. System-managed helpers (timestamps, flags) are written by code.
...
| **Pyscript reload** | HA integration reload | `state_bridge.py` re-seeds all 66 runtime sensors. Budget state restored from JSON. |
...
├── script/*.yaml                      35 script blueprints (features)
├── helpers_input_*.yaml              7 helper definition files (~225 entities)
