...
77 automation blueprints. 35 script blueprints. 35 pyscript modules exposing 155 services. 40 YAML packages. 22 voice pipelines across 3 languages. 582 helper entities. Zero lines of hand-written YAML. All code generated by **Claude** (Anthropic). All images generated by **Google Gemini**.
...
| Script blueprints | 35 |
| YAML packages | 40 |
| Pyscript modules | 35 (exposing 155 services) |
...
**Pipeline architecture:** 22 Assist Pipelines total. Each persona has Standard, Bedtime, Music Compose, and Music Transfer variants, plus Portuondo has a Therapy variant. Three HA-native fallback pipelines — Davis (English), Joana (Catalan), and Alvaro (Spanish) — provide budget-exhaustion fallback and multilingual coverage.
...
- **Tier 1 — Pyscript:** 35 Python modules exposing 155 services. Infrastructure: dispatcher, TTS queue, duck manager, memory, presence patterns, routine fingerprinting, music composition, budget tracking, system health, self-healing recovery, toggle audit, therapy session engine.
- **Tier 2 — Blueprints:** 77 automation + 35 script blueprints. User-facing features: bedtime routines, wake-up guards, notification routing, proactive briefings, music control, calendar CRUD, reactive banter, theatrical mode, therapy sessions.
- **Packages:** 40 YAML packages defining template sensors, automations, and helper wiring.
...
├── script/                        35 script blueprints
├── packages/                      40 YAML packages (sensors, automations, helpers)
├── pyscript/                      35 Python modules (155 services)
├── helpers/                       7 helper files (582 entities)
