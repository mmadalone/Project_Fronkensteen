...
| AP-55 | ❌ | `open()` or file I/O call in a regular pyscript function (outside `@pyscript_compile` or `@pyscript_executor`). Pyscript sandbox blocks `open()` — causes `NameError` crash at runtime. Use `@pyscript_executor` (preferred — auto-threads) for I/O functions; `@pyscript_compile` for pure-Python helpers or functions called from compiled bodies. **Additional subtlety:** `@pyscript_compile` functions called at **module level** still block the HA event loop (pyscript eval is synchronous for compiled calls). Move I/O to `@pyscript_executor` + async wrapper + `@time_trigger("startup")` instead. For files in `modules/` (regular Python, no pyscript decorators), wrap blocking calls from callers with `await asyncio.to_thread()`. | §10 #55 |
