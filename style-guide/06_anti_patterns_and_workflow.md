...
| AP-81 | ⚠️ | Detecting "is this a radio stream?" on a Music Assistant `media_player` by checking `media_content_type` alone. **MA reports `media_content_type: 'music'` for radio streams** (not `'radio'` as most developers assume). Any condition like `media_content_type == 'radio'` or `media_content_type in ['channel', 'radio', 'tvshow']` will silently fail for MA-managed radio and the code falls through to "track" handling. **Fix — three-signal detection:** (1) check `media_content_id` for the `library://radio/` prefix (MA-specific, authoritative); (2) keyword fallback: `'radio' in media_title \| lower or 'radio' in media_artist \| lower` **without** a `media_content_type == ''` gate (the original buggy guard); (3) optional `media_content_type in ['channel', 'radio', 'tvshow']` for non-MA radio integrations. Use all three OR'd together. Incident: `announce_music_follow_me_llm.yaml` v1.0–v1.x — the LLM prompt said "currently playing the track 'Radio Klara Lliure i Llibertaria'" on every music transfer because `media_content_type == 'music'` skipped the radio branch, and the fallback's `== ''` gate prevented the keyword match. Fixed 2026-04-09 (same session as Radio Klara feature). **Detection:** any Jinja or Python code that checks `media_content_type` against `'radio'` on an MA-backed media_player without also checking `media_content_id` or a title/artist keyword fallback. | §7.2 |
...
| AP-48 | ⚠️ | Direct `tts.speak` or `tts.cloud_say` call in a blueprint that should route through `pyscript.tts_queue_speak` (when TTS queue integration is present and the blueprint has a TTS queue toggle). **Critical:** Per [official HA docs](https://www.home-assistant.io/integrations/squeezebox/), `tts.speak` **automatically sets `announce: true` internally** — it is NOT a valid data key. Passing `"announce": true` in the `data:` dict causes `extra keys not allowed @ data['announce']` and silently kills TTS delivery. Valid `tts.speak` data fields: `message`, `media_player_entity_id`, `language`, `cache`, `options`. Use `tts_queue_speak` which accepts `announce` as an explicit parameter. | §14.8 |
...
| AP-55 | ❌ | `open()` or file I/O call in a regular pyscript function (outside `@pyscript_compile` or `@pyscript_executor`). Pyscript sandbox blocks `open()` — causes `NameError` crash at runtime. Use `@pyscript_executor` (preferred — auto-threads) for I/O functions; `@pyscript_compile` for pure-Python helpers or functions called from compiled bodies. **Additional subtlety:** `@pyscript_compile` functions called at **module level** still block the HA event loop (pyscript eval is synchronous for compiled calls). Move I/O to `@pyscript_executor` + async wrapper + `@time_trigger("startup")` instead. For files in `modules/` (regular Python, no pyscript decorators): (a) wrap blocking calls from callers with `await asyncio.to_thread()`; (b) if changing callers is infeasible, use `concurrent.futures.ThreadPoolExecutor` inside the module function — submit **only native CPython bound methods** (e.g. `path.exists`, `path.read_text`), never pyscript-defined functions (see AP-69). | §10 #55 |
| AP-56 | ⚠️ | `re.compile()` at module level or in a regular pyscript function (outside `@pyscript_compile`). The sandbox does NOT block `re.compile()` (only Python's built-in `compile()` and `open()` are in `BUILTIN_EXCLUDE`; `re` is in `ALLOWED_IMPORTS`), but the pyscript AST interpreter executes it significantly slower than native Python. Wrap pattern compilation in a `@pyscript_compile` function with `import re as _re` for performance — especially for patterns used in hot paths or compiled at module load time. | §10 #56 |
| AP-57 | ❌ | Generator expression (`any(x for x in ...)`, `all(...)`, `sum(... for ...)`) in any pyscript-interpreted function (regular or `async def` — anything without `@pyscript_compile` or `@pyscript_executor`). Pyscript throws `NotImplementedError: ast_generatorexp`. Use `any([x for x in ...])` list comprehension or explicit for-loops instead. | §10 #57 |
| AP-60 | ❌ | `import` statement inside a `try` block in a `@pyscript_executor` or `@pyscript_compile` function, where the imported module is referenced in an `except` clause (e.g., `except subprocess.TimeoutExpired`). If an earlier import in the same `try` block fails, the later module is never bound — Python raises `UnboundLocalError` at the `except` line. Move all imports that are referenced in `except` clauses **above** the `try` block. This is standard Python scoping, not a pyscript bug. | §10 #60 |
| AP-58 | ⚠️ | Template sensor `attributes:` block reads `state_attr()` from a media player (or similar entity) without guarding on the entity's `states()` being in an active state (`playing`/`paused`). Many integrations (Kodi, Apple TV, Chromecast, Sonos) retain stale attributes (`media_title`, `media_series_title`, etc.) after playback stops — the sensor exposes stale data to LLM agents and downstream consumers. Wrap each attribute in `{% if states(entity) not in ['playing', 'paused'] %}…{% endif %}`. | §3.6 |
| AP-69 | ❌ | Pyscript-defined function (from `modules/*.py` or any `.py` in `pyscript/`) passed to `ThreadPoolExecutor.submit()`, `threading.Thread(target=...)`, or any non-pyscript thread dispatch. Pyscript `modules/` files are run through pyscript's AST interpreter — functions defined there return **coroutine objects** when called outside pyscript's execution context (e.g., from a thread pool thread). Submit only native CPython objects: bound methods (`path.read_text`, `path.exists`), stdlib functions (`json.loads`, `yaml.safe_load`), or lambdas wrapping native calls. Parse results on the calling thread after `.result()`. | §10 #69 |
| AP-61 | ⚠️ | Large pyscript module silently fails to load when additional decorated functions (`@time_trigger`, `@state_trigger`, factory-created triggers) are appended. Root cause unknown — Python AST parse succeeds, file encoding is clean, and the same code works in a separate `.py` file. Observed with `agent_dispatcher.py` (~1580 lines): any idle monitor function appended to the file caused the entire module to silently not load (no error, no log, entity never created). Workaround: extract the function into a standalone pyscript module (e.g., `satellite_idle_reset.py`). Suspected pyscript file-size or AST-node-count limit. Discovered 2026-03-26. | §10 #61 |
| AP-70 | ❌ | `global` declaration inside a conditional branch (`if`/`else`/`elif`) in a pyscript function. Python treats the variable as local for the **entire** function once it sees `global` in any branch — reads before the branch execute raise `UnboundLocalError`. Always declare `global` at the **top** of the function body, before any reads. This is standard Python scoping but surfaces in pyscript because modules reload frequently during development. | §10 #70 |
| AP-71 | ❌ | Pyscript builtins (`event.fire()`, `log.info()`, `log.warning()`, `state.get()`, `state.set()`, `service.call()`) called from inside a `@pyscript_executor` or `@pyscript_compile` function, or from any plain function called by one. These builtins return **coroutine objects** in executor/compile context — they can't be awaited from a synchronous thread, causing `RuntimeWarning: coroutine never awaited`. Move all builtin calls to the async caller. For tracking functions that need to signal state transitions, return a value and let the async wrapper fire the event. | §10 #71 |
| AP-72 | ❌ | Blueprint offers TTS announcement without the full voice stack inputs: agent/pipeline selector, dispatcher toggle, presence sensor + satellite list, follow-me bypass toggle + claim/release scripts, TTS volume + announce mode, and LLM prompt. A bare `tts_queue_speak` call without these produces announcements on unknown speakers at unknown volume with no persona and no follow-me coordination. Reference: `email_follow_me.yaml` or `system_recovery_alert.yaml` v2.0.0. | §10 #72 |
| AP-73 | ❌ | `condition: template` with `value_template` used in a Lovelace **conditional card** `conditions:` block. Lovelace conditional cards only support `state`, `numeric_state`, `screen`, `user`, and `or` condition types — NOT `template`. The `condition: template` syntax is valid in automations/scripts but silently renders the card as "Configuration error" in the dashboard. Move template logic into the card's own content (e.g., `{% if ... %}` inside a markdown card body). | §10 #73 |
| AP-59 | ⚠️ | Text attribute sourced from Kodi (via REST sensor or `media_player` integration) missing `[COLOR]` markup cleanup. Kodi embeds `[COLOR xxxxxx]...[/COLOR]` rich text — including base64 item blobs like `[COLOR item="eJy..."]` — in metadata strings (`plot`, `title`, `label`). All Kodi-sourced text attributes must apply `\| regex_replace('\\\\[COLOR [^\\\\]]*\\\\]', '') \| regex_replace('\\\\[/COLOR\\\\]', '') \| trim` before use or truncation. | §3.6 |
| AP-74 | ⚠️ | New `input_boolean`/`input_text`/`input_number` helper created for runtime state (code-written flags, counters, state tracking). After Phase 3 Runtime State Migration (2026-03-29), all runtime state uses `state.set()` sensors — helpers are reserved for user-configured values only. For pyscript modules: use `state.set()` directly. For YAML automations/blueprints that can't call `state.set()`: use the `pyscript.set_sensor_value` bridge service (parameters: `entity_id`, `value`, `attrs_json`, `icon`, `friendly_name`). **Detection:** any new `input_*` helper definition where the value is only written by code (pyscript, automations) and never configured by the user. **Exception (Decision #94):** critical mutex/guard helpers that blueprints control via `input_boolean.turn_on/off` or `input_text.set_value` AND that must exist at boot before pyscript loads (e.g., `ai_handoff_processing`, `ai_handoff_pending`). | §10 #74, Decision #92, #94 |
| AP-75 | ⚠️ | `@pyscript_executor` or `service.call()` used inside a `@time_trigger("shutdown")` handler. During pyscript reload, the executor thread pool shuts down before `@pyscript_executor` functions complete, and `service.call()` dispatch may not finalize — both silently fail. **Fix:** Use `pathlib.Path.write_text()` for file I/O and inline `state.get()` (pyscript AST builtins) for state collection. Keep the handler non-async (no `await` — suspension points allow task cancellation). `pathlib.write_text()` produces a harmless HA "blocking call" warning. Pattern: `state_bridge.py` `_budget_shutdown_save()`. | §10 #75 |
| AP-79 | ⚠️ | Blueprint with `homeassistant.start` trigger calling a pyscript service without a startup delay. Pyscript `@time_trigger("startup")` handlers run concurrently with the `homeassistant.start` event. If the handler writes to the same sensor the blueprint's pyscript service writes to, `state.set(new_attributes=...)` (which replaces all attributes) creates a race — the handler can overwrite the service's results. **Fix:** Add a configurable `startup_delay` input (default 30s, exposed to user) and a conditional `delay:` action gated on `trigger.id == 'ha_start'`. Incident: `music_taste_rebuild.yaml` — genre summary lost on every restart because `_startup()` overwrote the rebuild result. | §10 #79 |
| AP-76 | ❌ | Blueprint `variables:` block with a **forward reference** — a variable whose `!input` default or template expression references another variable defined later in the same block. HA evaluates `variables:` sequentially; earlier entries cannot see later ones. The referenced variable resolves to `undefined` → Jinja renders it as empty string → `Template variable warning` in logs and incorrect runtime values (e.g., "Lights out in  minutes" instead of "Lights out in 4 minutes"). **Fix:** Reorder the `variables:` block so all dependencies (entity inputs, derived values like `countdown_minutes`) appear before the prompt/template variables that reference them. Incident: `bedtime_routine.yaml` / `bedtime_routine_plus.yaml` — `bedtime_prompt_tpl` referenced `countdown_minutes` (defined 100+ lines later), `v_countdown_system_prompt` referenced `bathroom_sensor` (defined 8 lines later). Also caught earlier in `proactive_bedtime_escalation.yaml` v1.2 changelog. | §10 #76 |
| AP-80 | ⚠️ | Pyscript module exposes a refresh/sync interval as a **hardcoded `cron(...)` decorator** when users would reasonably want to tune the cadence from a dashboard slider. `@time_trigger("cron(...)")` decorators are static — they cannot read helper values at decoration time. **Fix:** Use a fixed short tick (e.g. `cron(*/15 * * * *)`) and check `(now() - last_refresh_ts) >= input_number.<helper> * 3600` inside the function body. Trigger fires every 15 min but only does real work when the configured interval has elapsed. Stamp `last_refresh_ts` after each successful run; persist via `input_datetime` helper for restart survival. Also: pyscript reload may not re-fire `@time_trigger("startup")` if the module mtime hasn't changed — touch the .py file or restart HA when cache files need to propagate. Pattern: `pyscript/radio_klara.py` (`radio_klara_periodic` cron). | §10 #80 |
| AP-82 | ❌ | EOC `fallback_model` set to a model that doesn't reliably emit structured `tool_calls`. Symptom: agent says the in-character farewell ("alright, passing you to Rick") but the actual tool function never fires — handoff/escalation/end_conversation/etc. silently no-ops. Detection: `docker logs --since Xh homeassistant 2>&1 \| grep -c "No endpoints found that support tool use"` (Maverick 404 count) vs `\| grep -c 'finish_reason": "tool_calls"'`. If 404 count is non-zero AND tool_calls count is < 404 count, the fallback is dropping calls. The leaked tool call appears as a JSON literal inside `content`: `{"name":"handoff_agent","parameters":{...}}`. The 6-layer speech sanitizer in `conversation.py` Layer 5 (added 2026-04-08) strips this from TTS, perfectly hiding the failure. **Confirmed bad fallbacks:** `meta-llama/llama-3.3-70b-instruct`. **Confirmed good fallbacks:** `openai/gpt-4.1-mini`, `openai/gpt-4o-mini`, `anthropic/claude-haiku-4-5`. **Fix:** Update every EOC subentry's `fallback_model` field. Tool: `update_eoc_fallback.py` (PROJECT_DIR root) batch-edits `core.config_entries` with HA stopped. Original incident: voice handoff broke ~weeks before discovery 2026-05-01 because OpenRouter Maverick endpoint tool-use availability shrank gradually. | §14.5 |
| AP-83 | ⚠️ | Automation/blueprint condition that gates on a sensor that updates AFTER the trigger source fires, combined with `mode: restart` and a watchdog `for: "00:00:30"` trigger. Creates a silent failure window: real trigger fires → condition fails on stale sensor → 30 s later watchdog fires → its action clears the helper / does cleanup → real handoff/announcement/whatever never runs. Incident: `voice_handoff.yaml` checked `sensor.ai_last_satellite == satellite_entity` to disambiguate per-satellite blueprint instances. The sensor updates ~2 s AFTER the LLM tool fires (it's set when the response completes, not when the request starts). LLM tool path: tool fires `input_text.set_value` → flag_trigger fires → `ai_last_satellite` still stale → condition fails → 30 s later watchdog clears the pending helper → user heard the farewell quip but no handoff happened. **Fix:** Add an OR clause that reads live state of an entity that's already current at trigger time. For voice handoff: `or states(satellite_entity) in ['listening', 'processing', 'responding']`. **Generalisation:** when a sensor lags behind its trigger source, supplement the canonical condition with a synchronously-correct alternative read. | §14.5 |
| AP-84 | ❌ | Whole `state.attributes` dict assigned in a `variables:` block (or passed through a service `data:` field) without `\| to_json \| from_json`. HA 2026.6+ made nine core attribute keys a `StrEnum` (`homeassistant.const.EntityStateAttribute`: `assumed_state`, `attribution`, `device_class`, `entity_picture`, `friendly_name`, `icon`, `restored`, `supported_features`, `unit_of_measurement`). `StrEnum` keeps `__str__` but inherits `Enum.__repr__`, so `repr(EntityStateAttribute.ICON)` is `<EntityStateAttribute.ICON: 'icon'>` — and `dict.__repr__` uses `repr()` for keys. A `variables:` block **always** round-trips through text: render to string, then `ast.literal_eval()` inside `_parse_result()` (`helpers/template/__init__.py`). `literal_eval` raises on that repr, the `except` swallows it, and HA **silently returns the raw render string**. Nothing errors where the damage happens — the variable is now a `str` and only blows up at the first consumer with `UndefinedError: 'str object' has no attribute 'get'`, attributed to a step that may be hundreds of lines away. Detection: the `variables:` step renders **green** while a later step fails with `template_errors`; that mismatch is the signature. There is no log line at the corruption point, so grepping is futile. Live probe: add `"{{ _attrs is mapping }}"` — `False` means it was stringified. **Fix:** append `\| to_json \| from_json` (JSON coerces the enum keys back to plain `str`). **Filter order is load-bearing:** `\| default({})` must come BEFORE `to_json` — `states['missing'].attributes \| to_json` raises `TypeError: Type is not JSON serializable: LoggingUndefined`. **Round-trip is lossy for non-JSON types** (`datetime` → ISO string, tuple → list, `set`/`bytes`/`complex`/int ≥ 2^63 → hard `TypeError`); safe for JSON-sourced attrs (companion-app sensors), so prefer scalar `state_attr()` extraction on entities with exotic attribute values. **NOT a bug — do not "fix":** `{% set a = states['x'].attributes %}` inside a *single* template render never crosses the string boundary. Bonus: the round-trip also repairs `IntFlag` `supported_features` and `NaN` floats, which fail `literal_eval` for the same reason. Incident 2026-08-14: killed `notification_follow_me` (`_attrs` both branches, `_r_attrs`, `_dismiss_lr`) and `notification_replay` (`_attrs`) — dormant ~3 months, caught only by the trace watchdog. Verified on HA Core 2026.7.1 (`fixed=MAP unfixed=STR`). | §3.6 |
...
| AP-61 | ❌ | `pyscript.*` service call without `response_variable:` when the pyscript function returns data. HA validation raises `Script requires 'response_variable' for response data`. Applies to `memory_set`, `memory_search`, `agent_whisper`, `agent_dispatch`, and all pyscript services that return response payloads. Always declare `response_variable: _var_name` on these calls. | §3.0 |
| AP-62 | ❌ | `wait_for_trigger` with Jinja2 template in `entity_id:` field. HA state triggers require static entity IDs at parse time — `{{ variable }}` is treated as a literal string, not evaluated. Use `!input` for blueprint inputs or switch to `wait_template` for runtime-resolved entities. | §5.1 |
| AP-63 | ⚠️ | Kodi `media_player.play_media` with `media_content_type: CHANNEL` and a non-numeric `media_content_id` (e.g., plugin URI). Kodi integration calls `play_channel(int(media_id))` — raises `ValueError` that bypasses `continue_on_error`. Use `video` content type for plugin URIs, `CHANNEL` only for numeric PVR channel IDs. | §3.6 |
| AP-64 | ⚠️ | `assist_satellite.start_conversation` called with only `extra_system_prompt:` and no `start_message:` or `start_media_id:`. HA validation requires at least one of the latter two fields. Generate offer text via LLM first, then pass as `start_message`. | §3.0 |
| AP-65 | ❌ | Pyscript lazy-loaded cache from `.storage/` files permanently caches an empty result. After HA restart, `.storage` files may not be ready on first call — caching `personas: ()` permanently causes hours-long "unknown agent" errors. **Fix:** Use TTL-based refresh with a short retry interval (30s) for empty results. Never `return` early from `_ensure_cache()` when `_cache` is not `None` without checking TTL. Always provide a `_reload_cache()` function for manual invalidation. Pattern: `agent_dispatcher.py` lines 387-459 (Decision #69). | §10 #69 |
| AP-66 | ❌ | `initial:` on a runtime-state `input_number` / `input_text` / `input_datetime` helper (counters, accumulators, snapshots, timestamps). HA docs: *"If you set a valid value for `initial`, this integration will start with the state set to that value."* — i.e., every restart wipes the stored value. **Only use `initial:` for config/preference helpers** (rates, thresholds, toggles) that intentionally reset to a known default. Runtime-state helpers must omit `initial:` so HA restores the last value from the database. First-run safety: without `initial:`, HA sets state to `unknown` — consumers must handle this via `\| float(0)` / `\| int(0)` / `\| default(...)`. Incident: `initial: 0` on `ai_openrouter_usage_midnight` wiped the midnight snapshot on every restart, making daily cost = all-time total, which dropped budget to 0% and blocked all non-essential LLM calls. **Note (Phase 3, 2026-03-29):** Largely historical — 66 runtime state helpers migrated to `state.set()` sensors. Remaining `input_*` helpers are user-config only. See AP-74. | §10 #66 |
| AP-67 | ⚠️ | TTS text containing `[` characters bypasses `tts_queue.py` voice mood tag injection. The `"[" not in text` guard (line 935) prevents double-tagging agent responses, but also blocks mood tags for any text with incidental brackets (LLM stage directions like `[sighs]`, character performances, etc.). **Callers that generate bracket-containing text must prepend mood tags explicitly** before calling `tts_queue_speak` — read `input_text.ai_voice_mood_{agent}_tags` and prefix the text. tts_queue's guard will then correctly skip its own injection (sees `[` from the pre-applied tags). Pattern: `theatrical_mode.py` lines 718-725. | §14.8 |
| AP-68 | ⚠️ | Calling a `@pyscript_executor` function without a `callable()` guard. Executor registration can silently fail during module load (import errors, timing issues), leaving the function name bound to `None`. Calling `None(...)` produces `'<function>' is not callable (got None)` — a confusing crash with no pointer to the root cause. **Fix:** Before every `await executor_fn(...)` call, check `if not callable(executor_fn):` — log an actionable error and return a safe default. This is especially important for modules with 3+ executor functions (higher surface area for partial registration failure). Pattern: `theatrical_mode.py` `_ensure_cache()` guard. | §10 #68 |
| AP-69 | ❌ | Bare `state.get("entity.id")` for entities that may not exist in the HA state machine. Pyscript raises `NameError` (not returns `None`) for entities that were never registered — undocumented behavior contradicting pyscript docs. Conversation agents (`conversation.*`) and other integration-managed entities are common offenders. **Fix:** Always wrap in a `_safe_get()` helper: `try: return state.get(entity_id) except NameError: return None`. Also note: `"unknown"` is normal initial state for `input_text` (never set), `input_button` (no persistent state), and STT entities — do not treat as failure. Pattern: `system_health.py` `_safe_get()`. | §10 #69 |

| AP-78 | ❌ | Template sensor `attributes:` block reads from a `state.set()` pyscript sensor and a blueprint passes those attributes back to the pyscript service that writes the sensor — **circular dependency**. Stale data perpetuates because template reads pyscript value → blueprint passes it → pyscript writes it back unchanged. **Fix:** Template sensor attributes for fields that exist on the media_player (`series_title`, `season`, `episode`) must read from the media_player **only** — no fallback to the pyscript sensor. Pyscript-only data (`media_source`, `episode_name`) can still be read from the pyscript sensor since it's non-circular (doesn't exist on media_player, isn't passed back). Pyscript services should also read directly from the media_player via `state.getattr()` as defense-in-depth. **Incidents:** (1) `sensor.madteevee_now_playing` `series_title` stuck on "Night Court" while playing Scrubs — media_player was correct but template sensor preferred stale pyscript value (2026-04-01). (2) PVR channel switch: `series_title` stuck on "Wellington Paranormal" after changing channels — Kodi leaves `media_series_title` empty for PVR, so "media_player first" fallback still activated the pyscript path. Fixed by removing fallback entirely (2026-04-02). | §3.6 |
...
**Note on AP numbering:** IDs are non-sequential (AP-01 through AP-68 with gaps and sub-items like AP-10a, AP-25a). This is deliberate — IDs are stable references preserved from the original unified guide. Adding new anti-patterns gets the next available number; removing one retires the ID permanently (never reuse). Don't renumber — external references (changelogs, violation reports, build logs) depend on stable IDs.
...
84. **Never store a whole `state.attributes` dict in a `variables:` block (or service `data:` field) without `| to_json | from_json`.** On HA 2026.6+ the dict silently becomes a *string* and blows up later at the first `.get()` with `UndefinedError: 'str object' has no attribute 'get'`. Filter order matters: `| default({})` must come **before** `to_json`. Inside a single template render, `{% set a = states['x'].attributes %}` is safe and needs no round-trip — don't "fix" those.
...
81. **Never detect radio streams on MA by checking `media_content_type` alone.** Music Assistant reports `media_content_type: 'music'` for radio streams — not `'radio'`. Any `is_radio = media_content_type == 'radio'` check silently fails and code falls through to track handling. The authoritative signal is `media_content_id` starting with `library://radio/`. Always combine with a title/artist keyword fallback (without a `content_type == ''` gate, which is the original buggy pattern) for stations that don't set `library://radio/` on all playback paths. **Reference:** `announce_music_follow_me_llm.yaml` `is_radio` detection block, fixed 2026-04-09.
...
72. **Never add TTS to a blueprint without the full voice stack.** Any blueprint that offers a TTS announcement option MUST include: (a) agent/pipeline selector input, (b) dispatcher toggle (`use_dispatcher` + `dispatcher_enabled` entity), (c) presence sensor list + paired satellite list for speaker targeting, (d) follow-me bypass toggle + claim/release script inputs, (e) TTS volume + announce mode inputs, (f) an exposed LLM prompt for the announcement text. A bare `pyscript.tts_queue_speak` call without these inputs produces announcements on unknown speakers at unknown volumes with no persona and no follow-me coordination. Reference pattern: `email_follow_me.yaml` sections ①②⑤⑨⑪⑫, or `system_recovery_alert.yaml` v2.0.0 sections ①–⑥. **Discovered 2026-03-27** — `system_recovery_alert.yaml` v1.0.0 had a TTS toggle with a hardcoded `pyscript.tts_enqueue` call and no supporting inputs. **Detection:** any blueprint with `tts_queue_speak`, `tts.speak`, or `tts_enqueue` in its actions that lacks presence routing inputs AND dispatcher toggle inputs.
...
80. **Never hardcode a refresh/sync interval in a `@time_trigger("cron(...)")` decorator when users would reasonably want to tune it from a dashboard slider.** Pyscript `@time_trigger` decorators are static — they cannot read helper values at decoration time. Hardcoding `cron(*/30 * * * *)` permanently locks the cadence at 30 minutes; the `input_number.ai_*_refresh_*` helper for it (if any) is decorative. **Fix — fixed-tick + helper-check pattern:** use a fixed short cron (e.g. `cron(*/15 * * * *)` for hourly-or-coarser intervals; `cron(* * * * *)` only when minute precision matters) and check `(time.time() - _last_refresh_ts) >= _get_helper_hours() * 3600` inside the function body. The trigger fires every 15 min but only does real work when the configured interval has elapsed. Stamp `_last_refresh_ts = time.time()` after each successful run; persist the timestamp via an `input_datetime` helper for restart survival, and rehydrate `_last_refresh_ts` from `cached.get("fetched_at")` at startup. **Pattern:** `pyscript/radio_klara.py` `radio_klara_periodic` (Decision #96, 2026-04-09). Reusable for `project_promote.py`, `media_promote.py`, `calendar_promote.py`, etc. **Pyscript reload caveat:** `pyscript.reload` may not re-fire `@time_trigger("startup")` if the module mtime hasn't changed — touch the .py file or do a full HA restart when cache files need to propagate. **Detection:** any `@time_trigger("cron(...)")` decorator where the cron expression uses a non-trivial schedule (e.g. `cron(0 */N * * *)`, `cron(N 0 * * *)`) AND there exists no corresponding `input_number.*_refresh_*` helper that the function reads at runtime.

### Pyscript Sandbox Safety (55–57, 60, 65, 69)

55. **Never use `open()` or file I/O in a regular pyscript function.** The pyscript sandbox blocks built-in `open()` in interpreted context — calling it produces a `NameError` crash at runtime. **Use `@pyscript_executor`** for functions that do I/O — it compiles to native Python and auto-runs in a thread (no manual `task.executor()` needed). Call the function directly: `result = _foo_sync(args)`. Use `@pyscript_compile` only for pure-Python helpers (regex, json, string ops) that don't need threading, or when the function is called from another compiled function's body. Internal imports (`import json as _json`) are required inside both decorators. **Codebase status (2026-03-19):** 66 `@pyscript_executor` functions across 12 files; ~160 `@pyscript_compile` pure helpers. **For `modules/` files** where pyscript decorators are unavailable: (a) callers can wrap with `await asyncio.to_thread(fn)` — preferred when changing callers is feasible; (b) if zero caller changes is required, use `concurrent.futures.ThreadPoolExecutor` inside the function — but submit **only native CPython bound methods** (see AP-69). Reference: `shared_utils.py` `load_entity_config()` uses `ThreadPoolExecutor` with `_CONFIG_PATH.exists` / `_CONFIG_PATH.read_text` bound methods + thread detection to skip pool when already off the event loop. **Detection:** any `open(`, `Path(...).read_text()`, or `Path(...).write_text()` call outside a `@pyscript_compile` or `@pyscript_executor` function.
56. **Never use `re.compile()` at module level or in regular pyscript functions.** The pyscript sandbox may block regex compilation in interpreted context. Compile patterns inside a `@pyscript_compile` function using `import re as _re` and `_re.compile(...)`. Use lazy initialization to cache compiled patterns in a module-level variable on first use. **Detection:** any `re.compile(` call at module scope or inside a function without `@pyscript_compile`.
57. **Never use generator expressions in pyscript-interpreted functions.** Pyscript throws `NotImplementedError: ast_generatorexp` on any generator expression — `any(x for x in items)`, `all(...)`, `sum(... for ...)`, `sorted(... for ...)`, etc. This applies to **all** pyscript-interpreted functions: regular `def`, `async def`, and top-level code — anything without `@pyscript_compile` or `@pyscript_executor`. Use `any([x for x in items])` list comprehension or explicit for-loops instead. List comprehensions work fine; only bare generator expressions crash. **Survivor note (2026-03-24):** One instance survived the initial AP-57 scan — `any(... for ...)` in `agent_whisper.py` `_auto_update_keywords()`. Caused silent keyword auto-update failure for all agents. **Detection:** any `(... for ... in ...)` inside a function without `@pyscript_compile` or `@pyscript_executor`.
65. **Never permanently cache an empty result from `.storage/` file reads in pyscript.** After HA restart, `.storage/assist_pipeline.pipelines` and similar files may not be ready on first service call. If `_ensure_cache()` loads zero personas and caches the result with no TTL, all subsequent calls fail with "unknown agent" until pyscript reloads (hours-long error windows). **Fix:** Always use TTL-based refresh with a short retry interval (e.g., `_CACHE_EMPTY_RETRY = 30` seconds) for empty results and a normal TTL (e.g., 300-600s) for valid results. Provide a `_reload_cache()` function for manual invalidation and a corresponding `@service` for Developer Tools access. Warm the cache at startup via `await _ensure_cache()` in the `@time_trigger("startup")` function. **Reference pattern:** `agent_dispatcher.py` `_ensure_cache()` + `_reload_cache()` + `dispatcher_reload_cache()` service. **Detection:** any `_ensure_cache()` function that sets `_cache` to a dict without recording `_cache_ts = time.monotonic()`, or that returns early on `_cache is not None` without a TTL check.
60. **Never place `import` statements inside a `try` block when the imported module is referenced in an `except` clause.** If an earlier import in the same `try` block raises `ImportError`, later imports never execute — Python raises `UnboundLocalError` when the `except` clause references the unbound module (e.g., `except subprocess.TimeoutExpired` when `import subprocess` was inside the `try`). Move all imports referenced in `except` clauses **above** the `try` block. This is standard Python scoping ([CPython #35069](https://bugs.python.org/issue35069)), not a pyscript bug — but it surfaces most often in `@pyscript_executor` functions where multiple imports are grouped in a try/except. **Detection:** any `import X` inside a `try` block where `X.` appears in a sibling `except` clause.
69. **Never pass a pyscript-defined function to `ThreadPoolExecutor`, `threading.Thread`, or any non-pyscript thread dispatch.** Files in `pyscript/modules/` are run through pyscript's AST interpreter — they are NOT native Python. Functions defined there return **coroutine objects** when called from outside pyscript's execution context (e.g., a thread pool thread). This includes functions decorated with nothing, and even simple helpers. **Instead:** submit only native CPython bound methods (`path.exists`, `path.read_text`, `json.loads`), stdlib functions, or build-in type methods. Parse/transform results on the calling thread after `.result()` returns. **Discovered:** 2026-03-27 — `_read_config_file()` in `modules/shared_utils.py` was submitted to `ThreadPoolExecutor.submit()`, causing all 14 callers to crash with `'coroutine' object has no attribute 'get'`. Fixed by submitting `_CONFIG_PATH.exists` and `_CONFIG_PATH.read_text` (pathlib bound methods) instead. **Detection:** any `pool.submit(func_name)` or `Thread(target=func_name)` where `func_name` is defined in a `pyscript/modules/*.py` or `pyscript/*.py` file.
70. **Never declare `global` inside a conditional branch.** Python sees any `global X` in the function body and treats `X` as local for the **entire** function — including reads above the branch. If the branch with the `global` declaration doesn't execute, reads of `X` raise `UnboundLocalError`. Always place `global` declarations at the **top** of the function body, before any reads or conditional logic. This is standard Python scoping (not pyscript-specific) but surfaces often in pyscript because modules reload frequently during development and testing. **Discovered 2026-03-27** — `_ensure_cache()` in `agent_dispatcher.py` had `global _bypass_active` inside `if not personas:`, but the `else:` branch read `_bypass_active` — caused `UnboundLocalError` on every dispatch when cache loaded successfully. **Detection:** any `global` keyword that is NOT the first statement(s) in a function body, or any `global` inside an `if`/`else`/`elif`/`for`/`while`/`try` block.
71. **Never call pyscript builtins from `@pyscript_executor` or `@pyscript_compile` context.** `event.fire()`, `log.info()`, `log.warning()`, `state.get()`, `state.set()`, `service.call()`, and all other pyscript builtins return **coroutine objects** when invoked from executor/compile threads — they can't be awaited synchronously, causing `RuntimeWarning: coroutine 'EvalFuncVar.__call__' was never awaited`. This also applies to plain functions called transitively from executor context. **Fix:** return a signal value from the executor function, then fire events/log from the async caller. Example: `_track_write_failure()` returns `"read_only"` if mode transitioned, and the async wrapper fires `event.fire("ai_memory_mode_changed", ...)`. **Discovered 2026-03-27** — `memory.py` degradation tracking functions called `event.fire()` and `log.warning()` from inside `_memory_set_db_sync()` (`@pyscript_executor`), producing 2 coroutine warnings per memory write. **Detection:** any `event.fire(`, `log.`, `state.`, or `service.call(` inside a function decorated with `@pyscript_executor` or `@pyscript_compile`, or inside a function called from one.
73. **Never use `condition: template` in Lovelace conditional card `conditions:` blocks.** Lovelace conditional cards only support `state`, `numeric_state`, `screen`, `user`, and `or` condition types. The `condition: template` syntax with `value_template` is valid in automations and scripts but silently fails in dashboard cards — rendering the entire card as "Configuration error" with no diagnostic message. **Fix:** move template logic into the card's own content body (e.g., `{% if ... %}` inside a markdown card). For conditional visibility based on attribute values, create a template binary sensor and use `condition: state` on it. **Discovered 2026-03-27** — toggle audit dashboard card used `condition: template` to check if `last_entity` attribute was populated. Card rendered as "Configuration error" on every page load. **Detection:** any `condition: template` inside a `type: conditional` card's `conditions:` list in a dashboard YAML file.
...
5. **Header image — BLOCKING GATE (AP-15)** — If no header image exists, the referenced image file is missing from disk, or no image has been provided, you **must** ask the user before proceeding. Generate the image, present it for approval, and **wait for an explicit response** — either approval or "not necessary." **Do not write a single line of YAML until you get one of those answers.** If the user ignores the question or moves on to something else, **insist** — repeat the ask and explain that the style guide requires a clear answer before code generation begins. Use these defaults unless the user specifies otherwise:
   ...
   - **Character designs (canonical — do not deviate):**
     All header images are animated TV screenshots in Rick & Morty / Star Trek Lower Decks crossover style. Bold black outlines, cel-shaded coloring, vibrant saturated palette. Episode premises are based on the README/blueprint features. Include a 70s retro-styled title card at the bottom. Vary compositions — don't always use the same static shot.

     - **Rick Sanchez:** Blue spiky hair, unibrow, drool, white lab coat, glowing green device (scene-relevant), flask, smug half-lidded expression. Gray/pale skin.
     - **Quark (Ferengi):** SAME HEIGHT AS RICK — not short. Purple-gold ornate Ferengi tunic, gold medallion, large ridged ear lobes, sharp jagged teeth, orange-brown skin, holding tablet/PADD, scheming devious grin. Full body visible, standing upright, prominently visible.
     - **Deadpool:** Full red and black suit with mask, white eye patches, katana hilts visible on back. Comedic/irreverent energy. Thought bubble with tacos. Often thumbs-up or scene-relevant action.
     - **Kramer (Cosmo):** Tall, lanky build. Wild tall brown hair (messy, sticking up). Loud Hawaiian/patterned shirt (varies per image). Often bursting through a door. Startled/excited wide-eyed expression. Coffee mug sloshing.
     - **Dr. Portuondo:** Based on actor Jorge Perugorría (Cuban). STOCKY BROAD-SHOULDERED build — wide shoulders, barrel-chested, thick arms, fills the chair. NOT thin, NOT fat. Light olive skin (light-skinned Cuban, barely tanned). Gray-brown messy tousled hair (NOT white, NOT silver — warm gray-brown). Matching gray-brown beard, not too bushy. DARK NEAR-BLACK thick bushy eyebrows — dramatically darker than hair (signature contrast). Broad face, full cheeks, big nose, forehead wrinkles, heavy droopy eyelids. Round wire-frame spectacles low on nose. Dark olive-brown buttoned vest, white dress shirt, dark paisley tie with warm swirl patterns. Classic curved wooden pipe near mouth with blue-gray smoke. Glowing holographic clipboard on lap. Always seated in a brown tufted leather armchair. Expression: knowing, contemplative, furrowed brow — therapist observing chaos.
