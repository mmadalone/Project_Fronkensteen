...
| AP-78 | ❌ | Template sensor `attributes:` block reads from a `state.set()` pyscript sensor (e.g., `state_attr('sensor.ai_watch_history_status', 'current_series')`) as its **primary** source, with the media_player as fallback. A blueprint then reads those template sensor attributes and passes them to the pyscript service that writes back to the same `state.set()` sensor. This creates a **circular dependency**: stale data from a previous session perpetuates because the template sensor prefers the pyscript value, the blueprint reads it, and pyscript writes it back unchanged. **Fix:** Template sensor attributes for fields that exist on the media_player (`series_title`, `season`, `episode`) must prefer the media_player as primary source and fall back to the pyscript sensor only for enrichment data the media_player lacks (EPG episode names, source detection). Pyscript services should also read directly from the media_player via `state.getattr()` as defense-in-depth. Incident: `sensor.madteevee_now_playing` `series_title` stuck on "Night Court" while playing Scrubs — media_player was correct but template sensor preferred stale pyscript value. | §3.6 |
