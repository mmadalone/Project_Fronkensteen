...
| AP-56 | ❌ | `re.compile()` at module level or in a regular pyscript function (outside `@pyscript_compile`). Pyscript sandbox may block compiled regex in interpreted context. Wrap pattern compilation in a `@pyscript_compile` function with `import re as _re` and lazy-init the patterns. | §10 #56 |
