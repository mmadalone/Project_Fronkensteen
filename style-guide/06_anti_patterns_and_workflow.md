...
| AP-57 | ❌ | Generator expression (`any(x for x in ...)`, `all(...)`, `sum(... for ...)`) in a regular pyscript function. Pyscript throws `NotImplementedError: ast_generatorexp`. Use explicit for-loops or list comprehensions (`[x for x in ...]`) instead. | §10 #57 |
...
57. **Never use generator expressions in regular pyscript functions.** Pyscript throws `NotImplementedError: ast_generatorexp` on any generator expression — `any(x for x in items)`, `all(...)`, `sum(... for ...)`, `sorted(... for ...)`, etc. Use explicit for-loops or list comprehensions `[x for x in items]` instead. List comprehensions work fine; only bare generator expressions crash. **Detection:** any `(... for ... in ...)` inside a function without `@pyscript_compile`.
